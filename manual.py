...
def encrypt():
    while True:
        ...
        print(text)
        lens = lens + 1
        continu = input("would you like to continue? y/n, to print everything p, to export as json j \n")
        if continu == "n":
            break
        elif continu == "p":
            print(text)
...
def ask():
    ...
    if x == "e":
        ...
        print("please choose with e or d")
        ask()

def expjson():
    with open('decryptbl.json', 'w') as fp:
        json.dump(decryptbl, fp)
